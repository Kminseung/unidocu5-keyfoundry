...
define(function () {
    return function (initFn) {
        ...
        $magnachip.setGridHeaderDescription($u.gridWrapper.getGrid('UD_0220_132-grid2'), '*출장자는 지급 기준으로 작성 부탁드립니다.');
        ...
        gridObj2.setGroupHeader([
            ...
            {groupText: '식대/잡비', childColumns: ['DAY_EXP_A', 'DAY_EXP_T']},
        ...
        $.each(['ROOM_EXP_A', 'DAY_EXP_A', 'TRAN_EXP_A','ETC_EXP_A', 'SUM_EXP_A'], function (_, columnKey) {
        ...
        gridObj2.onChangeCell(function (columnKey, rowIndex, oldValue, newValue, jsonObj) {
            ...
            if (/LAND1/.test(columnKey) && jsonObj) {
                ...
                $magnachip.validateDestinaion(gridObj, gridObj2, 'LAND1', rowIndex)
        ...
        gridObj.onChangeCell(function (columnKey, rowIndex, oldValue, newValue) {
            function setWrbtrCalculated() {
                var fWrbtr
                if(gridObj.$V('WAERSF', rowIndex)==='JPY') fWrbtr= (Number(gridObj.$V('KURSF', rowIndex)) * Number(gridObj.$V('WRBTRF', rowIndex)))/100;
                    else  fWrbtr= Number(gridObj.$V('KURSF', rowIndex)) * Number(gridObj.$V('WRBTRF', rowIndex));
            ...
            if (/WAERSF$/.test(columnKey)) {
                ...
                myArray.map(function (val) {
                    ...
                    function findvalindex(element) {
                        if (element.INDEX === val) return true;
                    }

                    if (val.length > 0) {
                        arr.filter(function (element) {
                            val = gridObj.getJSONData()[rowIndex].INDEX;
                            if (val === element) {
                                var index = myArray.findIndex(findvalindex)
                                var gridData = gridObj2.getJSONData()[index]
                                if (newValue === 'USD') gridObj.$V('KURSF', rowIndex, gridData.USD_KURSF_TXT)
                                else if (newValue === 'EUR') gridObj.$V('KURSF', rowIndex, gridData.EUR_KURSF_TXT)
                                else if (newValue === 'JPY') gridObj.$V('KURSF', rowIndex, gridData.JPY_KURSF_TXT)
                                else if (newValue === 'HKD') gridObj.$V('KURSF', rowIndex, gridData.HKD_KURSF_TXT)
                                else if (newValue === 'CNY') gridObj.$V('KURSF', rowIndex, gridData.CNY_KURSF_TXT)
                                else if (newValue === 'TWD') gridObj.$V('KURSF', rowIndex, gridData.TWD_KURSF_TXT)
                                else if (newValue === 'KRW') gridObj.$V('KURSF', rowIndex, 1)
                            }
                        });
            ...
            if (columnKey === 'TRAN_TYPE' && gridObj.$V('EXP_GB',rowIndex)==='D') {
                ...
                if(/^A$|^B$/.test(rowData['TRAN_TYPE'])){
                    ...
                    var ediTableColumns = [];
                    ediTableColumns = ediTableColumns.concat('WAERSF', 'KURSF', 'WRBTRF');
            ...
            if (exp_gb === 'D') {
                var columnArray = ['KM', 'OIL','GAS_EXP', 'GAS_MIL', 'WRBTR','GAS_EXP']
                if (columnArray.indexOf(columnKey) !== -1) {
                    $magnachip.tranCalculate(gridObj,rowIndex,'WRBTRF')
                    setWrbtrCalculated();
                }
            }
        ...
        $u.buttons.addHandler({
            ...
            "addRow": function () {
                ...
                selectedjsonData.KURSF = Number(selectedjsonData.KURSF_TXT.replaceAll(',',''))
        ...
        $u.buttons.addCustomHandler({
            "calculateDate": function (columnKey,rowIndex) {
                ...
                if (nights >= 0) {
                    ...
                    $nst.is_data_tableParams_nsReturn($u.programSetting.getValue('CalculateFunction'), {}, {IT_HEAD: [activeRowJsonData]}, function (nsReturn) {
                        gridObj2.rg.style._setColumnStyles('KURSF','numberFormat','#,##0.00');
            ...
            "setGridExpense": function (data) {
                var mappingKey = {ROOM_EXP: 'A', DAY_EXP: 'B', TRAN_EXP: 'C',ETC_EXP: 'D'};
                ...
                $.each(data, function (columnKey, item) {
                    if ($u.util.contains(columnKey, ['ROOM_EXP', 'DAY_EXP', 'TRAN_EXP', 'ETC_EXP']) && Number(item) > '0') {
            ...
            'setSummaryFormValues': function () {
                function getSummaries(jsonData) {
                    ...
                    return jsonData.reduce(function (summaries, data) {
                        ...
                        var perLand = getOrCreate(perIndex, data['LAND1'], {});
                ...
                function setSummaries(summaries, mappingTable) {
                    ...
                    $.each(jsonData, function (_, data) {
                        ...
                        var perLand = perIndex ? perIndex[data['LAND1']] : undefined;
                    ...
                    $.each(jsonData, function (index, item) {
                        total = item['ROOM_EXP_A'] + item['DAY_EXP_A'] + item['ETC_EXP_A'] + item['TRAN_EXP_A'] + item['AIR_EXP_A'] + item['COM_EXP_A'] + item['ENT_EXP_A']
                        item['DAY_EXP_A'] = item['DAY_EXP_A'] + item['ETC_EXP_A']
                        item['TRAN_EXP_A'] = item['TRAN_EXP_A'] + item['AIR_EXP_A']
                        item['ETC_EXP_A'] = item['COM_EXP_A'] + item['ENT_EXP_A']
